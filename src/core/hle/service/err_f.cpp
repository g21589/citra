...
/**
 * ErrInfo:
 *  errtype0 : for Generic or Logged
 *  errtype3 : for Exception
 *  errtype4 : for Result Failure
 */
union ErrInfo {
    u8 specifier;
...
static std::string GetErrInfoType(u8 type_code) {
    ...
    default: return "Unknown ErrorInfo Type";
    }
}
...
static void ThrowFatalError(Service::Interface* self) {
    ...
    {
        const auto& errtype = errinfo->errtype0;

        // Generic Info
        LOG_CRITICAL(Service_ERR, "PID: 0x%08X",        errtype.pid);
        LOG_CRITICAL(Service_ERR, "REV: 0x%08X_0x%08X", errtype.rev_low, errtype.rev_high);
        LOG_CRITICAL(Service_ERR, "TID: 0x%08X_0x%08X", errtype.title_id_low, errtype.title_id_high);
        LOG_CRITICAL(Service_ERR, "AID: 0x%08X_0x%08X", errtype.app_title_id_low, errtype.app_title_id_high);
        LOG_CRITICAL(Service_ERR, "ADR: 0x%08X",        errtype.pc_address);
    ...
    case FatalErrType::Exception:
    {
        ...
        // Exception Info
        LOG_CRITICAL(Service_ERR, "EXCEPTION TYPE: %s", GetErrInfoType(errtype.exception_data.exception_info.exception_type).c_str());
