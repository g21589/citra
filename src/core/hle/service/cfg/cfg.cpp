...
ResultCode GetConfigInfoBlock(u32 block_id, u32 size, u32 flag, void* output) {
    auto pointer = GetConfigInfoBlockPointer(block_id, size, flag);
...
std::u16string GetUsername() {
    ...
    // so put copy it to a buffer to add trailing null char.
    char16_t username[ARRAY_SIZE(block.username) + 1] = {0};
